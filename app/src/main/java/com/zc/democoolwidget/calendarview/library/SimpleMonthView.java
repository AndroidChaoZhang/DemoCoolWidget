package com.zc.democoolwidget.calendarview.library;
...
//    public static final String VIEW_PARAMS_HEIGHT = "height";
    ...
    private boolean isValidNotDateBgShow = true;                               //出发日期到达日期之外的日期是否显示
    private boolean isStartAndEndDateFlag = true;                   //是否点击两次  一次出发时间  一次到达时间
    private static final int SELECTED_CIRCLE_ALPHA = 128;
    protected static int ROW_TOP_BOTTOM_PADDING = 5;                           // 一行的padding top bootom高度
    protected static int ROW_LEFT_RIGHT_PADDING = 3;                           // 一行的padding LEFT RIGHT高度
    ...
    private static int RIGHT_TEXT_SIZE;                                   // 右上角字体大小
...
//    private SimpleMonthAdapter.CalendarDay startCalendarDay;                 // 可选的出发选择日期
//    private SimpleMonthAdapter.CalendarDay endCalendarDay;                 // 可选的结束选择日期
//    private List<SimpleMonthAdapter.CalendarDay> mCalendarTags;         // 日期下面的标签
//    private String mDefTag = "标签";

    protected int mPadding = 0;// 日期控件的左右边距
...
//    protected Paint mTitleBGPaint;
    ...
    protected Paint mTextRightPaint;                    // 右上角的画笔
...
//    protected Boolean mDrawRect;              // 圆角还是圆形
    ...
    private Calendar todayCanendar;//当天的日历

    private final Calendar mCalendar;
    private final Calendar mDayLabelCalendar;           // 用于显示星期几
    private final Boolean isPrevAfterDayEnabled;             // 可操作日期之前和之后的日期是否能被操作   两个地方用到了  一个是onDayClick点击事件  一个是drawMonthCell绘画

    private int mNumRows;
    private DayPickerView.DataModel dataModel; //所有设定参数的model
    ...
    public SimpleMonthView(Context context, TypedArray typedArray, DayPickerView.DataModel dataModel) {
        ...
        todayCanendar = Calendar.getInstance();
...
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);
        ...
        RIGHT_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeRight, resources.getDimensionPixelSize(R.dimen.text_size_8sp));
        ...
        isPrevAfterDayEnabled = typedArray.getBoolean(R.styleable.DayPickerView_enablePreviousDay, false);
...
//        mDefTag = dataModel.defTag;
    ...
    private void onDayClick(SimpleMonthAdapter.CalendarDay calendarDay) {
        if (mOnDayClickListener != null && (isPrevAfterDayEnabled ||
                (!prevDay(calendarDay.day, todayCanendar) && !prevDay(calendarDay.day, dataModel.startCalendar) && !afterDay(calendarDay.day, dataModel.endCalendar)))) {
            mOnDayClickListener.onDayClick(this, calendarDay,isStartAndEndDateFlag);
        }
    }

    private boolean sameDay(int monthDay, Calendar calendar) {
     ...
     * @return
     */
    private boolean prevDay(int monthDay, Calendar calendar) {
        if (null!=calendar) {
            int year = calendar.get(calendar.YEAR);
            int month = calendar.get(calendar.MONTH);  //这里没有+1  是因为前面的日期都少1
            int day_of_month = calendar.get(calendar.DAY_OF_MONTH);
            return ((mYear < year)) || (mYear == year && mMonth < month) ||
                    (mYear == year && mMonth == month && monthDay < day_of_month);
        } else {
            return false;
        }
    }
     ...
     * 判断是否是已经过去的日期
     */
    private boolean afterDay(int monthDay, Calendar calendar) {
        if (null!=calendar) {
            int year = calendar.get(calendar.YEAR);
            int month = calendar.get(calendar.MONTH);  //这里没有+1  是因为前面的日期都少1
            int day_of_month = calendar.get(calendar.DAY_OF_MONTH);
            Log.e("MyLog","calendar"+year+month+day_of_month);
            Log.e("MyLog","monthDay"+mYear+mMonth+monthDay);
            return ((mYear > year)) || (mYear == year && mMonth > month) ||
                    (mYear == year && mMonth == month && monthDay > day_of_month);
        } else {
            return false;
    ...
    protected void drawMonthCell(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            // 可操作日期前后的日期
            boolean isPrevAfterDay = false;
            if (mHasToday && (mToday == day)) {//当天
                isToady = true;
                canvas.drawText("今天", x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                canvas.drawText("今", x+DAY_SELECTED_RECT_SIZE/2+ROW_LEFT_RIGHT_PADDING, y - DAY_SELECTED_RECT_SIZE/2 - ROW_TOP_BOTTOM_PADDING, mTextRightPaint);
            } else if (!isPrevAfterDayEnabled && (prevDay(day, todayCanendar) || prevDay(day,dataModel.startCalendar) || afterDay(day,dataModel.endCalendar))) {//不可操作的日期
                isPrevAfterDay = true;
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                ...
                if(isToady) {
                    canvas.drawText("今天", x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                    canvas.drawText("选", x+DAY_SELECTED_RECT_SIZE/2+ROW_LEFT_RIGHT_PADDING, y - DAY_SELECTED_RECT_SIZE/2 - ROW_TOP_BOTTOM_PADDING, mTextRightPaint);
                }
            }
...
//                canvas.drawText(String.format("%d", day), x, y - DAY_SELECTED_RECT_SIZE / 2, mDayTextPaint);
            ...
            for (SimpleMonthAdapter.CalendarDay calendarDay : dataModel.busyDays) {
                if (cellCalendar.equals(calendarDay) && !isPrevAfterDay) {
                    isBusyDay = true;
...
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
            ...
            for (SimpleMonthAdapter.CalendarDay calendarDay : dataModel.invalidDays) {
                if (cellCalendar.equals(calendarDay) && !isPrevAfterDay) {
                    isBusyDay = true;
//                    RectF rectF = new RectF(x - DAY_SELECTED_RECT_SIZE,
//                            (y - MINI_DAY_NUMBER_TEXT_SIZE / 3) - DAY_SELECTED_RECT_SIZE,
...
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
                    ...
                    canvas.drawText(String.format("%d", day), x, getTextYCenter(mTagTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                    canvas.drawText("禁", x+DAY_SELECTED_RECT_SIZE/2+ROW_LEFT_RIGHT_PADDING, y - DAY_SELECTED_RECT_SIZE/2 - ROW_TOP_BOTTOM_PADDING, mTextRightPaint);
                }
            }
            ...
            if (mStartDate != null && mEndDate == null && !mStartDate.equals(mEndDate) && !isInvalidDays && !isBusyDay && isValidNotDateBgShow) {
...
//                    canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
            ...
            // 绘制标签
            if (!isPrevAfterDay && !isInvalidDays && !isBusyDay && !isBeginDay && !isLastDay) {
                boolean isCalendarTag = false;
                for (SimpleMonthAdapter.CalendarDay calendarDay : dataModel.tags) {
                ...
                if (!isCalendarTag) {
                    canvas.drawText(dataModel.defTag, x, getTextYCenter(mTagTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
                }
            }

            // 绘制日期
            if (!isToady && !isPrevAfterDay && !isInvalidDays && !isBusyDay) {
                canvas.drawText(String.format("%d", day), x, getTextYCenter(mTagTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
            }
    ...
    public SimpleMonthAdapter.CalendarDay getDayFromLocation(float x, float y) {
        ...
        // 获取日期下面的tag
        boolean flag = false;
        for (SimpleMonthAdapter.CalendarDay calendarTag : dataModel.tags) {
...
//        mSelectedDayTextPaint.setFakeBoldText(true);
        ...
        mTextRightPaint.setColor(mDayTextColor);
        mTextRightPaint.setTextSize(RIGHT_TEXT_SIZE);
        mTextRightPaint.setStyle(Style.FILL);
        mTextRightPaint.setFakeBoldText(false);
    }
...
//            boolean isValidDay = false;
            ...
            for (SimpleMonthAdapter.CalendarDay day : dataModel.invalidDays) {
            ...
            for (SimpleMonthAdapter.CalendarDay day : dataModel.busyDays) {
                // 选择了入住日期，这时候比入住日期大且离入住日期最近的不可用日期变为可选
                if (calendarDay.equals(day) && !(mEndDate == null && mNearestDay != null && calendarDay.equals(mNearestDay))) {
                    return true;
                }
    ...
    public interface OnDayClickListener {
        void onDayClick(SimpleMonthView simpleMonthView, SimpleMonthAdapter.CalendarDay calendarDay, boolean isStartAndEndDateFlag);
    }
    ...
    private void drawDayBg(Canvas canvas, int x, int y, Paint paint) {
        RectF rectF = new RectF(x - DAY_SELECTED_RECT_SIZE - ROW_LEFT_RIGHT_PADDING, y - DAY_SELECTED_RECT_SIZE - ROW_TOP_BOTTOM_PADDING,
                x + DAY_SELECTED_RECT_SIZE + ROW_LEFT_RIGHT_PADDING, y + DAY_SELECTED_RECT_SIZE + ROW_TOP_BOTTOM_PADDING);
