...
import com.zc.democoolwidget.recycleview.excelcalendar.Pool;

/**
 * Created by qibin on 16-9-25.
 */

public class CardLayoutLXManager extends RecyclerView.LayoutManager {
    ...
    public CardLayoutLXManager(boolean center) {
        this(DEFAULT_GROUP_SIZE, center);
    }

    public CardLayoutLXManager(int groupSize, boolean center) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int firstLineSize = mGroupSize / 2 + 1;//每一组中第一行个数
        int secondLineSize = firstLineSize + mGroupSize / 2;//每一组中第二行个数
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            }else {//第二行的view的起始会有一个二分之一的item宽度的偏移
    ...
    private boolean isItemInFirstLine(int index) {//是否第一行显示
